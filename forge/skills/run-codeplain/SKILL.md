...
If verbose output is *not* showing up between the wrapper lines, the user has likely disabled it (`--no-log-to-file`, or a `--logging-config-path` that filters it). Confirm with the user and ask them to relaunch with verbose on; without the framework output, the classifier below is half-blind.
